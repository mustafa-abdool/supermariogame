...
def movebadguys(deltax): #maybe break it up into smaller functions, moves enemies
    for i in range(len(Enemies)): #makea  side rectangle, see if collides, if it does, reverse velocity...just put it INTO FUNCTION
        if Enemies[i]!=None: #MAYBE MAKE VARIABLES...MAKES THINGS EASIER TO UNDERSTAND
            ...
            siderect=Rect(Enemies[i][1][0]+32,Enemies[i][1][1],3,20)# represents rectangle at the side
            State=Statecheck(Enemies[i][1],siderect)
            if State=="Bounce":
                Enemies[i][2]=Enemies[i][2]*-1 #reverse velocity
            if State=="Falling":
                Enemies[i][1][1]=Enemies[i][1][1]+2 # make a tuple that invovles vertical nad horizontal movement
            else:
...
def Interactions(Rectobject,mario_size,y): #pass in mario_size, if nothing happens then we return it
   ...
   # print Obstacles
    ...
    draw.rect(screen,pink,bottom)
    display.flip()
    temp_enemies=[]
    for i in range(len(Enemies)):
        temp_enemies.append(Enemies[i][1])
    if bottom.collidelist(temp_enemies)!=-1: #WE HAVE TO CHANGE THIS LATER SO THAT DEATH ONLY OCCURS WITH MARIO RECT, BECAUSE HE COULD
        print "collision"
        pos=bottom.collidelist(temp_enemies) #BE FALLING ON MARIO, YO, AND HE CAN'T HIT IT FROM UNDERNEATH, CHECK IF ANYTHING ELSE COLLIDES FIRST, CHANGE THE RECTS AROUND A BIT
        if State=="Falling": #if he was standing ten the enemey dies, SOMETHING WRONG WITH THSI !!!!!!!!!!!!!!!!!!!!!!
            del Enemies[pos]
        if State=="Standing":
            print "dead" #if he was standing when a collision occured then he's dead 
...
def Statecheck(Rectobject,siderect):#almost like standing check except for obstacles
    platforms=[]
...
Enemies=[["goomba",Rect(300,300,32,32),3],["koopa",Rect(400,300,32,48),6]]        #[name,rectangle,speed]


Obstacles=twodim(10,2) #change this to 3 so that you can control the speed 
