...
#ENEMIES MOVE FASTER WHEN JUMPING
...
#MAYBE HAVE SIDERECT ALREADY SO WE DON'T HAVE TO FIGURE IT OUT EVERYTIME
    ...
    for i in range(len(Enemies)): #makea  side rectangle, see if collides, if it does, reverse velocity...just put it INTO FUNCTION
        if Enemies[i]!=None: #MAYBE MAKE VARIABLES...MAKES THINGS EASIER TO UNDERSTAND
           ...
           # siderect=Rect(Enemies[i][1][0]+30,Enemies[i][1][1],3,30)
            draw.rect(screen,pink,Enemies[i][1])
...
def Interactions(Rectobject,mario_size,y): #pass in mario_size, if nothing happens then we return it
   ...
   # print Obstacles
    ...
    elif mario_rect.collidelist(temp_enemies)!=-1: #if anyother part of mario collides then he's dead 
        print "dead"
    ...
    for object1 in Obstacles:
     #   print object1
        ...
        if object1[1]=="mushroom": #always subtracting Mario's original position
            State=Statecheck(object1[0],object1[0]) #Create a SPEED VARIABLE that way if the mushroom hits a wall if will "bounce off"
            if State=="Moving": #FIX THE CODE ABOVE THIS...YOU HAVE TO MAKE A SIDE RECTTTT YOU MUSTTTTTTTT!!!!
...
Enemies=[["goomba",Rect(300,300,32,32),3],["koopa",Rect(450,300,32,48),6]]        #[name,rectangle,speed]
