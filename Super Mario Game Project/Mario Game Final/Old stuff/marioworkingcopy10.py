...
#YOU CAN JUMP THROUGH WALLS W/ JUS TTHE SPACE BAR...BECAUSE IT CHECKS LEFT/RIGHT FIRST..MAKE A FLAG MAYBE?
...
big_pic_list=[mario5,mario6,mario7]

big_jumplist=[image.load("Super Mario Jump.png")]

bigstandlist=[image.load("Super Mario Stand.png")]
...
def Sizecheck(screen,x,y,False,mx,my,deltax,Action): #gets the picture list of whatever mario is doing
    if mario_size=="small":
        if Action=="standing":
            return drawMario(screen,x,y,standing,False,mx,my,deltax,mario_size)
       # if Action=="jumping":
   ...
   # print Obstacles
    ...
    if Rectobject.collidelist(temp_obstacles)!=-1:
        ...
        if Obstacles[pos][1]=="flower":
            print "monicaaa" #works but the rect doesn't >_>
        if Obstacles[pos][1]=="mushroom":
            #print "collide"
            del Obstacles[pos]
            mario_size="Super"
         # print "moose"
...
def jump(x,y,intial_velocity,delta_x,gravity_change,pictures,boolean,mx,my,oldx,mario_size):  #mario size represents what stage he's in # make intial velocity negative, boolean represents left jump image
    ...
    #for i in range...maybe make a range and go through it, might make collisions better 
        ...
        if State=="Standing":
            if mario_size=="small":
                drawMario(screen,x,y,standing,False,mx,my,delta_x,mario_size) #if standing then we stop 
                return mx,my,x,y
            if mario_size=="Super":
                drawMario(screen,x,y,bigstandlist,False,mx,my,delta_x,mario_size) #if standing then we stop 
                return mx,my,x,y                
        #print State
        if State=="Bounce": # if he hit the underside then we just jump again but this time with no velocity,a nd high gravity to make him fall#
            if mario_size=="small":
                return jump(x,y,0,delta_x,5,jumplist,boolean,mx,my,oldx,mario_size) #delta_x makes him bouncee
            if mario_size=="Super":
                 return jump(x,y,0,delta_x,5,big_jumplist,boolean,mx,my,oldx,mario_size) #delta_x makes him bouncee
        if State=="Bounce2" or State=="Bounce3":
            if mario_size=="small":
                return jump(x,y,-2,0,7,standing,boolean,mx,my,oldx,mario_size) #makes him fall really fast, if he collides w/ the side
            if mario_size=="Super":
                return jump(x,y,-2,0,7,bigstandlist,boolean,mx,my,oldx,mario_size) 
        Deathcheck(x,y)
       # print (x,y)
  ...
  #  print State
    ...
    if State=="Standing" or State=="Bounce2" or State=="Bounce3": #Bounce2 and Bounce3 are collision states
        ...
        if key.get_pressed()[K_SPACE]==True and key.get_pressed()[K_RIGHT]==True:
           ...
           Facing="Right"
           if mario_size=="Super":
               mx,my,x,y=jump(x,y,-10,horizontal_movement,gravityforce,big_jumplist,False,mx,my,deltax,mario_size) #boolen is for left flag
           if mario_size=="small":
               mx,my,x,y=jump(x,y,-10,horizontal_movement,gravityforce,jumplist,False,mx,my,deltax,mario_size) #boolen is for left flag
        elif key.get_pressed()[K_SPACE]==True and key.get_pressed()[K_LEFT]==True:
           # changemusic(0,1)
            Facing="Left"
            if mario_size=="Super":
                mx,my,x,y=jump(x,y,-10,horizontal_movement*(-1),gravityforce,big_jumplist,True,mx,my,deltax,mario_size)
            if mario_size=="small":
                mx,my,x,y=jump(x,y,-10,horizontal_movement*(-1),gravityforce,jumplist,True,mx,my,deltax,mario_size)
           ...
           # changemusic(0,1)
            ...
            if Facing=="Left":
                ...
                movement=-1 
            if mario_size=="Super":
                mx,my,x,y=jump(x,y,-10,movement,gravityforce,big_jumplist,leftflag,mx,my,oldx,mario_size) 
            if mario_size=="small":
                mx,my,x,y=jump(x,y,-10,movement,gravityforce,jumplist,leftflag,mx,my,oldx,mario_size) 
            ...
            deltax=x-oldx
            if mario_size=="Super":
                mario_size=drawMario(screen,x,y,big_pic_list,False,mx,my,deltax,mario_size) #for run animation
            if mario_size=="small":
                mario_size=drawMario(screen,x,y,pic_list,False,mx,my,deltax,mario_size) #for run animation 
            ...
            deltax=x-oldx
            if mario_size=="Super":
                mario_size=drawMario(screen,x,y,big_pic_list,True,mx,my,deltax,mario_size) #for run animation
            if mario_size=="small":
                mario_size=drawMario(screen,x,y,pic_list,True,mx,my,deltax,mario_size) #by returning mario_size you get super or not  

   # print deltax
    if x==oldx: #if he's not moving we just draw it, this ways the obstacles can move tooo
        if mario_size=="small":
            mario_size=drawMario(screen,x,y,standing,leftflag,mx,my,0,mario_size)
        if mario_size=="Super":
            mario_size=drawMario(screen,x,y,bigstandlist,False,mx,my,0,mario_size) #for run animation
   # else:
