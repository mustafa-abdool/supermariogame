...
def movebadguys(deltax): #maybe break it up into smaller functions, moves enemies
    ...
    for i in range(len(Enemies)): #makea  side rectangle, see if collides, if it does, reverse velocity...just put it INTO FUNCTION
        if Enemies[i]!=None: #MAYBE MAKE VARIABLES...MAKES THINGS EASIER TO UNDERSTAND
           # old=Enemies[i][1][0]
            Enemies[i][1][0]=Enemies[i][1][0]-deltax
            if Jumpflag==True:
                #if Enemies[i][0]=="koopa":
                if count%2==0: #CONTROLS THE DELAY 
    ...
    for fire in range(len(fireball_list)):
     #   print fireball_list[fire]
        if fireball_list[fire]!=None:
            ...
            State=Statecheck(fireball_list[fire],fireball_list[fire],fireball_list[fire])
            if State=="Moving":
                fireballgravity1=0
            if State=="Bounce":
...
def makeplatforms(): #makes some platforms 
    ...
    return list1,barriers,platforms2
...
Enemies=[["koopa",Rect(450,300,32,48),10,(32,48),False],["goomba",Rect(300,300,32,32),2,(32,32),False]]        #[name,rectangle,speed,dimensions,reverseflag]
...
bothplatforms,barriers,top_plats=makeplatforms() #both platforms contains both the rect and the texture in a 2d list 
...
while running:
   ...
   # print sidebarriers
    ...
    if State=="Standing" or State=="Bounce2" or State=="Bounce3": #Bounce2 and Bounce3 are collision states
        ...
        elif key.get_pressed()[K_f]==True and mario_size=="Fire": #have a counter for this, checks only when its pressed
            print "Fire" 
